...
a/使用的时候要记得指针的长度。
B/malloc的时候得确定在那里free.
C/对指针赋值的时候应该注意被赋值指针需要不需要释放.
D/动态分配内存的指针最好不要再次赋值.
...
调用拷贝构造函数的三种场景：
1当出现对象的等号赋值时，会调用拷贝函数；
2当出现把对象做函数参数进行传递时，会调用拷贝构造函数；
3对象做函数返回值时，会调用拷贝构造函数。
...
2、调用数据成员对象或者基类的构造函数，且这个构造函数中有参数需要初始
class Base  
{  
public:  
...
限制只在堆上分配内存：
即只能用new,不能直接定义。做法：将析构函数设为私有，类对象就无法建立在栈上了，如果直接使用A  a 来建立对象，编译报错，提示析构函数无法访问，只能使用new操作符来建立对象。缺点1：必须自己手动打造destory函数来析构对象，而不能用delete。     缺点2：析构函数做成私有的话，无法正常继承---》析构函数写出protected的
对比

缺点：1、new和destory搭配问题
2、继承问题
